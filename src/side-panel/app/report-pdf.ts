...
const softInk: Rgb = [107, 114, 128];
const border: Rgb = [218, 226, 236];
const faintBorder: Rgb = [235, 240, 246];
const paper: Rgb = [250, 252, 255];
const accent: Rgb = [31, 91, 190];
const linkBlue: Rgb = [37, 99, 235];
const critical: Rgb = [190, 18, 60];
const warning: Rgb = [194, 102, 20];
...
function drawCover(layout: PdfLayout, input: BuildReportPdfInput): void {
...
function drawSectionTitle(layout: PdfLayout, title: string, subtitle?: string): void {
  const document = layout.document;
...
function drawScanContext(layout: PdfLayout, input: BuildReportPdfInput): void {
...
function drawSeveritySummary(layout: PdfLayout, violations: readonly KodeGlassViolation[], totalFindings: number): void {
  const document = layout.document;
...
function drawEngineStatusTable(layout: PdfLayout, statuses: AccessibilityReport['engineStatuses']): void {
  const document = layout.document;
...
function drawRemediationPlan(layout: PdfLayout, violations: readonly KodeGlassViolation[]): void {
...
function drawAppendixIntro(layout: PdfLayout, findingCount: number, evidenceCount: number): void {
  drawText(
    layout,
...
async function drawFindingEntry(
  layout: PdfLayout,
...
function drawEmptyState(layout: PdfLayout, text: string): void {
  layout.ensureSpace(34);
...
function addPageNumbers(document: jsPDF, layout: PdfLayout): void {
...
function severityRgb(severity: ViolationSeverity): Rgb {
  if (severity === 'critical') {
    return critical;
  }
...
function statusRgb(status: AccessibilityReport['engineStatuses'][number]['status']): Rgb {
  if (status === 'completed') {
    return success;
  }
